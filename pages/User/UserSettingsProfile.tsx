  ...
  return (
    <div className="grid grid-cols-4 gap-x-8 py-4">
      <div className="flex w-full items-center gap-x-2">
        ...
        <Input label="Nickname (required)" name="nickname" onChange={handleChange} value={user?.nickname || ""} />
        <div className="mb-1 h-8 w-8 self-end">
          <Button
            icon={IconEnum.save}
            isDisabled={userData?.nickname === user?.nickname}
            onClick={undefined}
            variant="success"
          />
        </div>
      ...
      <div className="flex w-full items-center gap-x-2">
        <Input label="Email (required)" name="email" onChange={handleChange} value={user?.email || ""} />
        <div className="mb-1 h-8 w-8 self-end">
          <Button icon={IconEnum.save} isDisabled={userData?.email === user?.email} onClick={undefined} variant="success" />
        </div>
      </div>
    </div>
  );
