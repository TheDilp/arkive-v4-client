import { MentionAtomChangeHandler, MentionAtomNodeAttributes } from "@remirror/extension-mention-atom";
import { ChangeReason, SuggestChangeHandlerProps } from "@remirror/pm/suggest";
import { MenuNavigationOptions, useMenuNavigation, UseMenuNavigationReturn } from "@remirror/react";
...
export function useMentionAtom<Data extends MentionAtomNodeAttributes = MentionAtomNodeAttributes>(
  ...
  const { ignoreMatchesOnDismiss = true, items, direction, dismissKeys, focusOnClick, submitKeys, replacementType } = props;
  ...
  const onChange: MentionAtomChangeHandler = useCallback(
    (prps, command) => {
      const { query, range, name, exitReason, changeReason, text } = prps;
